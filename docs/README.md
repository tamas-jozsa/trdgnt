...
Runs once per day at **9:00 AM ET**. Analyses a 34-ticker curated watchlist
...
9:00 AM ET
     ...
     |
     +-- for each of 34 tickers (parallel analysis available):
           ...
           Research Manager    → synthesises all → investment plan  [gpt-4o]
...
| `--no-wait` | off | Skip the 9:00 AM ET wait, run immediately |
...
| `--parallel N` | `1` | Parallel workers for analysis (1-4, default 1 = sequential) |

---

## Parallel Analysis Mode

The system supports parallel analysis for faster cycle completion:
...
**How it works:**
- **Phase 1 (Parallel):** All tickers are analyzed simultaneously using process pools
- **Phase 2 (Sequential):** Trades are executed one at a time to avoid race conditions

**Safety measures:**
- Portfolio state is refreshed between each trade execution
- Checkpoint updated after each ticker completion
- 10-minute timeout per ticker analysis
- Errors are isolated per ticker
...
An optional asyncio-based daemon for real-time news monitoring:
...
**Features:**
- Polls Reuters, Finnhub, and Reddit every 5 minutes
- LLM-based triage to identify material news events
- Auto-triggers analysis for affected tickers
- Dashboard integration for start/stop control

**Configuration:** Edit `news_monitor_config.py` to adjust:
- Poll interval (default: 5 minutes)
- Urgency thresholds for triggering analysis
- Source enablement (Reuters, Finnhub, Reddit)

---

## Web Dashboard

A local web dashboard provides real-time monitoring at `http://localhost:8080`:
...
**Pages:**
- **Portfolio** — Equity curve, open positions, sector exposure
- **Trades** — Trade history with performance metrics
- **Agents** — Full agent reasoning viewer per ticker
- **Research** — Research findings browser
- **Control** — System control panel + live log feed

**Stack:** FastAPI (Python) + React (TypeScript) + WebSocket live updates
